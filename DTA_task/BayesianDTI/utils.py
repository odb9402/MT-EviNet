...
def get_mse_coef(gamma, nu, alpha, beta, y, tau=1.):
    alpha_eff = check_mse_efficiency_alpha(gamma, nu, alpha, beta, y).mean()
    nu_eff = check_mse_efficiency_nu(gamma, nu, alpha, beta, y).mean()
    delta = (gamma - y).mean()
    min_bound = torch.min(nu_eff, alpha_eff)#.mean().detach()
    c = (min_bound/delta).detach().mean() * tau**2
    return torch.clip(c, min=False, max=1.).detach()

def get_mse_coef_test(gamma, nu, alpha, beta, y, batch_reduce='mean'):
    """
    Generate the coefficient of the MSE loss, to clip the gradient.
    We call the maximum gradient value "the clipping value".

    U_a = check_mse_efficiency_alpha(gamma, nu, alpha, beta, y) 
    U_v = check_mse_efficiency_nu(gamma, nu, alpha, beta, y) 

    U = mean( min(U_a, U_v) )

    Where U is the clipping value.

    Args:
        gamma, nu, alpha, beta[torch.Tensor (N,1)] output values of the evidential network
    Returns:
        [torch.Tensor (N,1)]: Mse coefficients to clip the gradient as "U". 
    """
    alpha_eff = check_mse_efficiency_alpha(gamma, nu, alpha, beta, y).detach()
    nu_eff = check_mse_efficiency_nu(gamma, nu, alpha, beta, y).detach()
    delta = (gamma - y).abs().detach()
    if batch_reduce == 'mean':
        min_bound = torch.min(nu_eff, alpha_eff).mean()
...
def get_multi_obj_coef(model, objective_a, objective_b, a_args, b_args, **kwargs):
    ...
    return get_mtl_min_norm(grad_a, grad_b)
...
def check_mse_efficiency_nu(gamma, nu, alpha, beta, y):
    ...
    #return torch.clip(beta*(nu_1+1)*(nu+1)/alpha, min=0).detach()
    #return torch.clip(beta*(nu_1+1)*nu_1, min=0).detach()
